...
if project_slug[0].isdigit():
    print(
        f"{ERROR_COLOR}ERROR: {project_slug=} is not a valid name for this template. "
        f"Project names should not start with a number.{RESET_ALL}"
    )
    sys.exit(1)
...
if not all(char in valid_characters for char in project_slug):
    print(
        f"{ERROR_COLOR}ERROR: {project_slug=} is not a valid name for this template. "
        f"Project names should only contain lowercase letters, numbers, hyphens, "
        f"and underscores.{RESET_ALL}"
    )
    sys.exit(1)
...
if os.path.isdir(project_slug):
    print(
        f"{ERROR_COLOR}ERROR: A directory with the name '{project_slug}' already "
        f"exists in the current directory. Please choose a different project name.{RESET_ALL}"
    )    
    sys.exit(1)
